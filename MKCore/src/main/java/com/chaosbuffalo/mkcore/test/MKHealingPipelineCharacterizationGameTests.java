...
public class MKHealingPipelineCharacterizationGameTests {
    ...
    @GameTest(template = "player_data_phase0")
    public static void parameterizedHealingEffectAppliesRuntimeHealBonus(GameTestHelper helper) {
        Player caster = createMockPlayer(helper, CASTER_POS);
        Player target = createMockPlayer(helper, TARGET_POS);
        target.setHealth(4.0f);
        ...
        AbilityFormula healingFormula = AbilityFormula.bonusScaledLinear(
                TEST_HEAL_BASE,
                TEST_HEAL_PER_LEVEL,
        ...
        helper.startSequence()
                .thenExecuteAfter(2, () -> {
                    ...
                    assertFloatEquals(helper, healed, 7.0f, 0.001f,
                            "parameterized healing should add runtime heal bonus through the shared pipeline");
                })
                .thenSucceed();
    }
