...
public class ShotgunProjectileBehavior extends ProjectileCastBehavior{
    protected final LocationProvider locationProvider;
    public static final Codec<ShotgunProjectileBehavior> CODEC = RecordCodecBuilder.<ShotgunProjectileBehavior>mapCodec(builder -> builder.group(
            LocationProvider.CODEC.fieldOf("location").forGetter(i -> i.locationProvider)
    ).apply(builder, ShotgunProjectileBehavior::new)).codec();

    public ShotgunProjectileBehavior(LocationProvider provider) {
        this.locationProvider = provider;
    }

    @Override
    public ProjectileCastBehaviorType<? extends ProjectileCastBehavior> getType() {
        return ProjectileCastBehaviorTypes.LINE.get();
    }
    ...
    public void startCast(ProjectileAbility ability, IMKEntityData casterData, int castTime, AbilityContext context) {
        ...
        for (int i = 0; i < locationProvider.getMax(); i++) {
