...
public record MKTowerStairProfile(MKWorkspaceStairMode mode, MKWorkspaceStairRiseType riseType, int cycleLength,
    ...
    public static MKTowerStairProfile forTemplateReuse(MKWorkspaceStairAuthoringConfig config, int shaftWidth, int shaftLength) {
        MKWorkspaceStairMode mode = normalizeMode(config.mode());
        if (mode == MKWorkspaceStairMode.LADDER || mode == MKWorkspaceStairMode.NONE) {
            return new MKTowerStairProfile(mode, config.riseType(), 1, 2, 0, 1);
        }
        ...
        return new MKTowerStairProfile(mode, config.riseType(), cycleLength, riseHalfBlocksPerStep,
    ...
    public static List<Integer> getAllowedHeights(MKWorkspaceStairMode requestedMode, int hallwayWidth, int minimumHeight,
                                                  int count) {
        MKTowerStairProfile profile = forTemplateReuse(
    ...
    public static List<Integer> getAllowedFlatRunLengths(MKWorkspaceStairAuthoringConfig config, int shaftWidth,
        ...
        while (candidate <= maxCandidate && values.size() < count) {
            ...
            MKTowerStairProfile profile = forTemplateReuse(candidateConfig, shaftWidth, shaftLength);
