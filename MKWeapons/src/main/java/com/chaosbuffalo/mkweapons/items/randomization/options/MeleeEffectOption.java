...
public class MeleeEffectOption extends EffectOption<IMeleeWeaponEffect> {
    ...
    public static final MapCodec<MeleeEffectOption> MAP_CODEC = RecordCodecBuilder.<MeleeEffectOption>mapCodec(builder -> {
    ...
    public static final Codec<MeleeEffectOption> CODEC = MAP_CODEC.codec();
    ...
    public void applyToItemStackForSlot(ItemStack stack, LootSlot slot, double difficulty) {
        getItemEffects().forEach(eff -> {
            ...
            MeleeEffectsComponent.addEffect(stack, copied);
        });
    }
}