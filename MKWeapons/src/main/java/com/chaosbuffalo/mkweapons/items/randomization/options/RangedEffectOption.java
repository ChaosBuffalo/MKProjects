...
public class RangedEffectOption extends EffectOption<IRangedWeaponEffect> {
    public static final ResourceLocation NAME = MKWeapons.id("ranged_effect");
    public static final MapCodec<RangedEffectOption> MAP_CODEC = RecordCodecBuilder.<RangedEffectOption>mapCodec(builder -> {
    ...
    public static final Codec<RangedEffectOption> CODEC = MAP_CODEC.codec();
    ...
    public void applyToItemStackForSlot(ItemStack stack, LootSlot slot, double difficulty) {
        getItemEffects().forEach(eff -> {
            ...
            RangedEffectsComponent.addEffect(stack, copied);
        });
    }
}
